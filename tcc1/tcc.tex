...
% Estrutura: 4 capítulos (Introdução, Referencial Bibliográfico,
...
% ---------------------------------------------------------------
\begin{resumo}
Este documento apresenta a proposta inicial do Trabalho de Conclusão de Curso (TCC1) desenvolvido no SENAC Santo Amaro. O objetivo deste trabalho é [descrever brevemente o objetivo principal do TCC]. A justificativa para este estudo baseia-se em [descrever brevemente a justificativa]. O referencial bibliográfico abrange [mencionar os principais tópicos]. Como solução proposta, pretende-se [descrever brevemente a solução]. Este trabalho está organizado em capítulos que apresentam a introdução ao tema, o referencial bibliográfico que fundamenta a pesquisa, o desenvolvimento com a solução proposta e o cronograma de trabalho, seguidos das referências bibliográficas.
...
% GLOSSÁRIO
% ---------------------------------------------------------------
% Em ordem alfabética, defina os termos técnicos relevantes do
% trabalho (conceitos, não meras siglas). Diferentemente da Lista
% de Abreviaturas e Siglas, o glossário traz o SIGNIFICADO/definição
% de cada termo. Observação: pela ABNT NBR 14724 o glossário também
% pode aparecer como elemento PÓS-textual (após as referências);
% aqui ele é apresentado antes da Introdução, conforme orientação
% da disciplina. Edite os itens conforme o seu projeto.
\pdfbookmark[0]{Glossário}{glo}
\chapter*{Glossário}

\noindent O glossário a seguir reúne, em ordem alfabética, os principais termos técnicos empregados neste trabalho, acompanhados de suas definições. Substitua, acrescente ou remova itens conforme a necessidade do seu projeto.

\vspace{\onelineskip}

\begin{description}
  \item[Backend] Camada da aplicação responsável pela lógica de negócio, pelo processamento dos dados e pela comunicação com o banco de dados.
  \item[Endpoint] Ponto de acesso de uma API, identificado por uma URL, que responde às requisições enviadas pelo cliente.
  \item[Framework] Estrutura de software reutilizável que oferece funcionalidades genéricas e padroniza o desenvolvimento de aplicações.
  \item[Frontend] Camada da aplicação com a qual o usuário interage diretamente, responsável pela interface e pela experiência de uso.
  \item[Protótipo] Versão preliminar e funcional de um sistema, construída para validar conceitos e requisitos antes da implementação definitiva.
  \item[Repositório] Local centralizado onde o código-fonte e o seu histórico de versões são armazenados e gerenciados.
\end{description}
...
\begin{itemize}
    ...
    \item \textbf{Reserve o aprofundamento técnico} para o Referencial Bibliográfico e o Desenvolvimento --- a introdução prepara o terreno, não esgota o assunto.
\end{itemize}
}}
...
% ===============================================================
\chapter{Referencial Bibliográfico}
\label{cap:referencial}

O referencial bibliográfico tem como finalidade apresentar o embasamento teórico e técnico do trabalho, demonstrando o conhecimento do autor sobre o tema e identificando o estado da arte. Para evitar dúvidas sobre o que escrever em cada parte, este capítulo está organizado em apenas três seções: a \textbf{metodologia da pesquisa bibliográfica} (como as fontes foram levantadas e selecionadas), o \textbf{referencial} (os conceitos e as tecnologias que fundamentam a proposta, reunidos em um único lugar) e os \textbf{trabalhos relacionados} (estudos que enfrentaram o mesmo problema).
...
Esta seção reúne, em um único lugar, todo o embasamento teórico e técnico do trabalho. Aqui você apresenta os conceitos, teorias e definições que fundamentam a pesquisa e, na sequência, detalha as tecnologias, algoritmos, frameworks ou metodologias específicas que serão empregados no desenvolvimento da solução. \textbf{Não é necessário separar ``teoria'' de ``fundamentos técnicos''}: organize o conteúdo de forma fluida, partindo do mais geral para o mais específico.

Organize esta seção por temas ou conceitos, não por autor. Cada subseção deve abordar um conceito relevante, explicando-o, relacionando-o ao contexto do trabalho e, quando aplicável, mostrando como ele será utilizado na solução proposta.

Por exemplo:
\begin{itemize}
    \item Se o trabalho envolve desenvolvimento web, apresente desde os conceitos gerais (arquitetura cliente-servidor, APIs REST, padrões de projeto) até as tecnologias concretas que serão usadas (linguagens, frameworks, bancos de dados) e o porquê de cada escolha;
    \item Se envolve inteligência artificial, explique os fundamentos da área e, em seguida, os algoritmos e técnicas específicos que serão aplicados (árvores de decisão, redes neurais, SVM, etc.);
    \item Se envolve análise de dados, apresente os conceitos estatísticos e, na sequência, as métricas e técnicas que serão efetivamente utilizadas.
\end{itemize}

Utilize citações para fundamentar cada conceito apresentado e empregue figuras, diagramas e tabelas para ilustrar quando apropriado. Segundo \citeonline{Wazlawick2020}, o referencial deve demonstrar que o autor domina os fundamentos necessários para conduzir a pesquisa.
...
\begin{figure}[htb]
    \centering
    \caption{Exemplo de inserção de figura --- substitua pela imagem da sua solução (diagrama de arquitetura, fluxograma ou mockup).}
    \label{fig:exemplo_arquitetura}
    \includegraphics[width=0.35\textwidth]{assets/logo-senac.png}
    \legend{Fonte: Elaborado pelo autor (2025)}
\end{figure}

Toda figura deve ser numerada, ter legenda (\verb|\caption|) e indicação de fonte, além de ser citada no texto --- por exemplo, a Figura~\ref{fig:exemplo_arquitetura}. As figuras com legenda aparecem automaticamente na \textit{Lista de ilustrações}, nos elementos pré-textuais.
...
% ---------------------------------------------------------------
\section{Apresentação oral (banca)}
\label{sec:apresentacao}

Além do documento escrito, o TCC1 é avaliado por uma \textbf{apresentação oral} perante a banca. Um template pronto em \texttt{Beamer} está disponível no arquivo \texttt{apresentacao\_tcc1.tex}, na mesma pasta deste documento.

\noindent\fbox{\parbox{\dimexpr\textwidth-2\fboxsep-2\fboxrule}{%
\textbf{Regras da apresentação do TCC1}

\vspace{0.2cm}
\begin{itemize}
    \item \textbf{Duração: 20 minutos.} Cronometre os ensaios; ultrapassar o tempo é penalizado. Reserve uma margem de 2 a 3 minutos para perguntas da banca dentro desse total.
    \item \textbf{Siga a mesma estrutura do documento escrito}, nesta ordem: Introdução (contexto, justificativa, objetivos), Referencial Bibliográfico (metodologia da pesquisa, referencial, trabalhos relacionados) e Desenvolvimento (solução proposta e cronograma).
    \item \textbf{Ritmo:} cerca de 1 a 1,5 minuto por slide, totalizando aproximadamente 14 a 18 slides de conteúdo (fora capa, roteiro e encerramento).
    ...
    \item \textbf{Conteúdo dos slides:} poucos tópicos por slide, frases curtas; prefira diagramas, tabelas e imagens a parágrafos longos.
    \item \textbf{Slides de apoio (backup):} inclua slides extras após o ``Obrigado'' com detalhes técnicos e tabelas completas para responder à banca.
\end{itemize}

\vspace{0.2cm}
\textbf{Distribuição de tempo sugerida (20 min):} Introdução $\approx$ 5 min; Referencial Bibliográfico $\approx$ 5 min; Desenvolvimento $\approx$ 7 min; encerramento $\approx$ 3 min.
}}
